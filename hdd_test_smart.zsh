...
function check_hdd_type(){
	if [[ $HDDType == "sata" ]] || [[ $HDDType == "ide" ]] || [[ $HDDType == "scsi" ]] || [[ $HDDType == "sas" ]] || [[ $HDDType == "sat1" ]] || [[ $HDDType == "sat2" ]]; then
...
function smartctl_watched(){
	if [[ $HDDType == "sata" ]] || [[ $HDDType == "sas" ]] || [[ $HDDType == "ide" ]]; then
		sudo smartctl --device=auto --smart=on --saveauto=on --tolerance=normal --test=long /dev/$1 1>/dev/null;
		SmartKill+=(${1} 0);
		Locks+=(${1} 1);
		Queue+=(${1} 2);
	elif [[ $HDDType == "scsi" ]]; then
		sudo smartctl --device=scsi --captive --smart=on --saveauto=on --tolerance=normal --test=long /dev/$1 1>/dev/null &;
		local SmartctlPidInit=$!;
		sleep 1;
		local SmartctlPidLimit;
		local SmartctlPidLimit=$(( $SmartctlPidInit + 50 ));
		local SmartctlPidSet=0;
		local SmartctlPidPs="$(sudo ps $SmartctlPidInit | awk '/sudo smartctl/' | awk '{print substr($0,28,102)}')";
		while (( $SmartctlPidSet == 0 )) && (( $SmartctlPidInit <= $SmartctlPidLimit )); do
			if [[ $SmartctlPidPs == "sudo smartctl --device=auto --captive --smart=on --saveauto=on --tolerance=normal --test=long /dev/$1" ]] && (( $SmartctlPidInit < $SmartctlPidLimit )); then 
				SmartctlScsiPid+=(${1} $SmartctlPidInit);
				SmartctlPidSet=1;
				Queue+=(${1} 2);
				SmartKill+=(${1} 0);
				Locks+=(${1} 1);
			elif [[ $SmartctlPidPs != "sudo smartctl --device=auto --captive --smart=on --saveauto=on --tolerance=normal --test=long /dev/$1" ]] && (( $SmartctlPidInit < $SmartctlPidLimit )); then 
				(( SmartctlPidInit = $SmartctlPidInit + 1 ));
				SmartctlPidPs="$(sudo ps $SmartctlPidInit | awk '/sudo smartctl/' | awk '{print substr($0,28,102)}')";
			else
				sudo smartctl --device=scsi --smart=on --saveauto=on --tolerance=normal --test=long /dev/$1 1>/dev/null &;
				Locks+=(${1} 1);
				SmartKill+=(${1} 0);
				SmartctlScsiPid+=(${1} 0);
				SmartctlPidSet=1;
		...
		local SmartctlPidPs="$(sudo ps $SmartctlPidInit | awk '/sudo smartctl/' | awk '{print substr($0,28,102)}')";
		while (( $SmartctlPidSet == 0 )) && (( $SmartctlPidInit <= $SmartctlPidLimit )); do
			if [[ $SmartctlPidPs == "sudo smartctl --device=scsi --captive --smart=on --saveauto=on --tolerance=normal --test=long /dev/$1" ]] && (( $SmartctlPidInit < $SmartctlPidLimit )); then 
				SmartctlScsiPid+=(${1} $SmartctlPidInit);
				SmartctlPidSet=1;
				Queue+=(${1} 2);
				SmartKill+=(${1} 0);
				Locks+=(${1} 1);
			elif [[ $SmartctlPidPs != "sudo smartctl --device=scsi --captive --smart=on --saveauto=on --tolerance=normal --test=long /dev/$1" ]] && (( $SmartctlPidInit < $SmartctlPidLimit )); then 
				(( SmartctlPidInit = $SmartctlPidInit + 1 ));
				SmartctlPidPs="$(sudo ps $SmartctlPidInit | awk '/sudo smartctl/' | awk '{print substr($0,28,102)}')";
			else
				sudo smartctl --device=scsi --smart=on --saveauto=on --tolerance=normal --test=long /dev/$1 1>/dev/null &;
	...
	elif [[ $HDDType == "sat1" ]] || [[ $HDDType == "sat2" ]]; then
		sudo smartctl --device=sat,auto --smart=on --saveauto=on --tolerance=normal --test=long /dev/$1 1>/dev/null;
		SmartKill+=(${1} 0);
		Locks+=(${1} 1);
		Queue+=(${1} 2);
	else
...
function check_smartctl_test_status(){
	if [[ $HDDType == "sata" ]] || [[ $HDDType == "ide" ]] || [[ $HDDType == "sat1" ]] || [[ $HDDType == "sat2" ]]; then
	...
	elif [[ $HDDType == "sas" ]]; then
		local TestStatus="$(sudo smartctl -a /dev/${1} | awk '/Self-test execution status:/' | awk '{print substr($0,34,17)}')";
		local SasTestStatus="$(sudo smartctl -a /dev/${1} | awk '/# 1  Background long/' | awk '{print substr($0,24,9)}')";
		local LogStatus="$(sudo smartctl -H /dev/${1} | awk '/SMART Health Status: / {print substr($0,0,24)}')";
		if [[ ${TestStatus} == *"f test remainin"* ]]; then
			Queue+=(${1} 2);
		elif [[ ${TestStatus} !=  *"f test remainin"* ]] && [[ ${LogStatus} == "SMART Health Status: OK" ]] && [[ ${SasTestStatus} == "Completed" ]]; then
			Queue+=(${1} 3);
		else
			Queue+=(${1} 20);
			if (( $Error[$1] == 0 )); then
	...
	elif [[ $HDDType == "scsi" ]] && (( $SmartctlScsiPid[$1] != 0 )); then
		local SmartctlPid=$SmartctlScsiPid[$1];
		local SmartctlPidPs="$(sudo ps $SmartctlPid | awk '/sudo smartctl/' | awk '{print substr($0,28,102)}')";
		local SmartctlHealth="$(sudo smartctl -H /dev/${1} | awk '/SMART Health Status:/')";
		local SmartTestLogFG="$(sudo smartctl -a /dev/${1} | awk '/Foreground long/' | head -n 1 | awk '{print substr($0,6,27)}')";
		if [[ $SmartctlPidPs == "sudo smartctl --device=scsi --captive --smart=on --saveauto=on --tolerance=normal --test=long /dev/$1" ]]; then 
			Queue+=(${1} 2);
		elif [[ $SmartctlPidPs != "sudo smartctl --device=scsi --captive --smart=on --saveauto=on --tolerance=normal --test=long /dev/$1" ]] && [[ ${SmartctlHealth} == "SMART Health Status: OK" ]] && [[ $SmartTestLogFG == "Foreground long   Completed" ]]; then
			Queue+=(${1} 3);
		else
			Queue+=(${1} 20);
			if (( $Error[$1] == 0 )); then
		...
		local SmartTestLogBG="$(sudo smartctl -a /dev/${1} | awk '/Background long/' | head -n 1 | awk '{print substr($0,6,27)}')";
		local SmartctlHealth="$(sudo smartctl -H /dev/${1} | awk '/SMART Health Status:/')";
		if [[ ${SmartctlHealth} == "SMART Health Status: OK" ]] && [[ $SmartTestLogBG == "Background long   Self test" ]]; then
			Queue+=(${1} 2);
		elif [[ ${SmartctlHealth} == "SMART Health Status: OK" ]] && [[ $SmartTestLogBG == "Background long   Completed" ]]; then
			Queue+=(${1} 3);
		else
			Queue+=(${1} 20);
			if (( $Error[$1] == 0 )); then
...
function get_hdd_data(){
	if (( $DriveDataCollected[$1] == 0 )) && (( $Presence[$1] == 1 )) && (( $Queue[$1] != 14 )) ; then
		...
		elif [[ $HDDType == "scsi" ]] || [[ $HDDType == "sas" ]]; then
			local SmartSupportRaw="$(sudo smartctl -a /dev/${1} | awk '/SMART support is:/' | awk '{print substr($0,23,9)}; NR == 1 {exit}')";
			local ModelFamilyRaw="$(sudo smartctl -a /dev/${1} | awk '/Vendor:/' | awk '{print substr($0,23,35)}')";
			local ModelRaw="$(sudo smartctl -a /dev/${1} | awk '/Product:/' | awk '{print substr($0,23,35)}')";
			local SerialNumberRaw="$(sudo smartctl -a /dev/${1} | awk '/Serial number:/' | awk '{print substr($0,23,35)}')";
			local UserCapacityGB="$(sudo smartctl -a /dev/${1} | awk '/User Capacity:/' | awk -F"[" '{print $2}' | grep -o '[0-9]\+' | head -n 1)"
			local DateTime="$(date +%s)";
			StartTime+=(${1} $DateTime);
		fi
...
main(){
	clear;
	if [[ ${1} == "" ]] || [[ ${1} == "sata" ]] || [[ ${1} == "scsi" ]] || [[ ${1} == "sas" ]] || [[ ${1} == "sat1" ]] || [[ ${1} == "sat2" ]] || [[ ${1} == "ide" ]]; then
		print "INVOCATION ERROR: Batch Name not defined";
		print "Try invoking the command with the following syntax."; 
		print "<./hdd_test.zsh batchname HDDType>";
		print "\"batchname\" can be anything the user desires.";
		print "\"HDDType\" can be sata, sas, sat1, sat2, ide.";
	elif [[ ${1} != "" ]]; then 
		if [[ ${2} == "sata" ]] || [[ ${2} == "scsi" ]] || [[ ${2} == "sas" ]] || [[ ${2} == "sat1" ]] || [[ ${2} == "sat2" ]] || [[ ${2} == "ide" ]]; then
			check_hdd_type;
			if [[ ${Batch} != '' ]]; then
			...
			print "INVOCATION ERROR: HDD Type not defined or is incorrect";
			print "Try invoking the command with the following syntax."; 
			print "<./hdd_test.zsh batchname HDDType>";
			print "\"batchname\" can be anything the user desires.";
			print "\"HDDType\" can be sata, sas, sat1, sat2, ide.";
